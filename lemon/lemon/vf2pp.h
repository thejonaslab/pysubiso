...
namespace lemon {
  ...
  class Vf2pp {
    ...
    template<MappingType MT>
    bool extMatch(double max_duration_sec = 0.0){
      // max_duration_sec == 0 means "don't check"
      ...
      while(_depth>=0) {
        ...
        loop_counts++; 
        size_t CHECK_TIME_EVERY = 1000; 
        if (((loop_counts % CHECK_TIME_EVERY)  == 0) &
            (max_duration_sec > 0)) {
            // check time periodically
            auto time_end = std::chrono::steady_clock::now();
            auto runtime_duration_ns = std::chrono::duration_cast<std::chrono::nanoseconds> (time_end - time_start);

            if ((float(runtime_duration_ns.count())/1e9) > max_duration_sec) {
                std::cout << "extMatch an out of time, max_duration_sec=" << max_duration_sec << std::endl;
  ...
  public:
    ...
    bool find(double max_duration_sec=0.0)
    {
      switch(_mapping_type)
        {
        case SUBGRAPH:
          return extMatch<SUBGRAPH>(max_duration_sec);
        case INDUCED:
          return extMatch<INDUCED>(max_duration_sec);
        case ISOMORPH:
          return extMatch<ISOMORPH>(max_duration_sec);
        default:
          return false;
        }
    }
  ...
  public:
    ...
    bool run(double max_duration_sec=0.0) {
      ...
      const bool ret = alg.find(max_duration_sec);
